...
class SimuladorVentas:
    ...
    def simular_dia(self):
        ...
        for i in range(self.n_clientes):
            ...
            for k in range(n_hotdogs):
                ...
                falta = None
                for ingrediente in list(hotdog.values())[1:]:
                  if not ingrediente:
                      continue
                      ...
                      for item in ingrediente:
                          if not item:
                              continue
                          elif not self.inventario.revisar_stock(item):
                              falta = item
                  else:
                ...
                if falta:
                    ...
                    self.ingredientes_fallo.add(falta)
                    break
                    ...
                    for ingrediente in list(hotdog.values())[1:]:
                        if not ingrediente:
                            ...
                            for item in ingrediente:
                                if not item:
                                    continue
                                self.inventario.restar_stock(item, cantidad=1)
                        else:
                            self.inventario.restar_stock(ingrediente, cantidad=1)
                    # Actualizar el reporte de ventas
                    self.stats[hotdog_vendido["nombre"]]["ventas"] += 1
    ...
    def generar_reporte(self):
        ...
        print(f"Hot dogs que no tuvieron stock: {[hotdog for hotdog in self.stats if self.stats[hotdog]['fallos'] > 0]}")
